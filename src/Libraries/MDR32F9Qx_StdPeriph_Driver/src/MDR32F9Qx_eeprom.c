...
/** @defgroup EEPROM_Private_Defines EEPROM Private Defines
  * @{
  */
float FlashFreqMHz = FLASH_PROG_FREQ_MHZ;
...
/* KN 29.04.2016 */
#define DELAY_LOOP_CYCLES               (16UL)	
//float Loop_us = FLASH_PROG_FREQ_MHZ / DELAY_LOOP_CYCLES;
...
__RAMFUNC void ProgramDelay(uint32_t Loops) __attribute__((section("EXECUTABLE_MEMORY_SECTION")));
...
/**
  ...
  * @retval None.
  */
__RAMFUNC void ProgramDelay(uint32_t Loops)
{
...
__RAMFUNC void ProgramDelay_us(uint32_t Loops) __attribute__((section("EXECUTABLE_MEMORY_SECTION")));
...
/**
  ...
  * @retval None.
  */
__RAMFUNC void ProgramDelay_us(uint32_t Loops) 
{
	for (; Loops > 0; Loops--)
		{ }
}
...
/**
  * @brief  Erases one sector of the selected EEPROM memory bank.
...
__RAMFUNC void EEPROM_EraseSector(uint32_t Address, uint32_t BankSelector)
{
  ...
  //for (Offset = 0; Offset < (4 << 2); Offset += 4)
  {
    //MDR_EEPROM->ADR = Address + Offset;             /* Page Address */
		MDR_EEPROM->ADR = 0x8000000 + (Address & 0x1F00C);
    MDR_EEPROM->DI = 0;
